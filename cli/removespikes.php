...
if (file_exists('./include/global.php') & !file_exists('../rrdtool-proxy.php')) {
	...
	switch ($arg) {
		...
		case '-A':
			if ($value == 'avg') {
				...
				print "FATAL: You must specify either 'avg' or 'nan' as replacement methods.\n\n";
				display_help();
				exit(-10);
			}
		...
		case '-O':
			...
			if (!is_numeric($outliers) || ($outliers < 1)) {
				print "FATAL: The number of outliers to exclude must be a positive integer.\n\n";
				display_help();
				exit(-6);
			}
...
if (!$using_cacti) {
	...
	if ($response !== false && strlen($response['stdout'] . $response['stderr'])) {
		...
		print "FATAL: RRDTool not found in path.  Please insure RRDTool can be found in your path!\n";
		exit(-1);
	}
}
...
if ($html) {
	print "<table cellpadding='3' cellspacing='0' class='spikekill_data' id='spikekill_data'>";
}
...
if (file_exists($xmlfile)) {
	...
	if ($using_cacti) {
		print ($html ? "<tr><td colspan='20' class='spikekill_note'>" : '') . 'FATAL: RRDtool Command Failed.  Please verify that the RRDtool path is valid in Settings->Paths!' . ($html ? "</td></tr>\n" : "\n");
	} else {
		print ($html ? "<tr><td colspan='20' class='spikekill_note'>" : '') . 'FATAL: RRDtool Command Failed.  Please insure your RRDtool install is valid!' . ($html ? "</td></tr>\n" : "\n");
	}
	exit(-12);
}
...
if ($backup && !$dryrun) {
	if (copy($rrdfile, $bakfile)) {
		print ($html ? "<tr><td colspan='20' class='spikekill_note'>" : '') . "NOTE: RRDfile '$rrdfile' backed up to '$bakfile'" . ($html ? "</td></tr>\n" : "\n");
	} else {
		print ($html ? "<tr><td colspan='20' class='spikekill_note'>" : '') . "FATAL: RRDfile Backup of '$rrdfile' to '$bakfile' FAILED!" . ($html ? "</td></tr>\n" : "\n");
		exit(-13);
	}
}
...
*/
print ($html ? "<tr><td colspan='20' class='spikekill_note'>" : '') . "NOTE: Searching for Spikes in XML file '$xmlfile'" . ($html ? "</td></tr>\n" : "\n");
...
if ($method == 1) {
	...
	if ($std_kills) {
		...
		print ($html ? "<tr><td colspan='20' class='spikekill_note'>" : '') . "NOTE: NO Standard Deviation Spikes found in '$rrdfile'" . ($html ? "</td></tr>\n" : "\n");
	}
} else {
	// variance subroutine
	if ($var_kills) {
		...
		print ($html ? "<tr><td colspan='20' class='spikekill_note'>" : '') . "NOTE: NO Variance Spikes found in '$rrdfile'" . ($html ? "</td></tr>\n" : "\n");
	}
}
...
if (!$dryrun) {
	if ($total_kills) {
		if (writeXMLFile($new_output, $xmlfile)) {
			...
			print ($html ? "<tr><td colspan='20' class='spikekill_note'>" : '') . "FATAL: Unable to write XML file '$xmlfile'" . ($html ? "</td></tr>\n" : "\n");
		}
	}
} else {
	print ($html ? "<tr><td colspan='20' class='spikekill_note'>" : '') . 'NOTE: Dryrun requested.  No updates performed' . ($html ? "</td></tr>\n" : "\n");
}
...
function backupRRDFile($rrdfile) {
	...
	print ($html ? "<tr><td colspan='20' class='spikekill_note'>" : '') . "NOTE: Backing Up '$rrdfile' to '" . $backupdir . '/' . $newfile . "'" . ($html ? "</td></tr>\n" : "\n");
...
function display_help() {
	...
	print 'Cacti Spike Remover ' . ($using_cacti ? 'v' . $version['version'] : $version) . ", Copyright 2004-2026, The Cacti Group, Inc.\n\n";
	print "Usage:\n";
	print "removespikes.php -R|--rrdfile=rrdfile [-M|--method=stddev] [-A|--avgnan] [-S|--stddev=N]\n";
	print "                 [-P|--percent=N] [-N|--number=N] [-D|--dryrun] [-d|--debug]\n";
	print "                 [--html] [-h|--help|-v|-V|--version]\n\n";

	print "The RRDfile input parameter is mandatory.  If no other input parameters are specified the defaults\n";
	print "are taken from the Spikekill Plugin settings.\n\n";

	print "-M|--method      - The spike removal method to use.  Options are 'stddev'|'variance'\n";
	print "-A|--avgnan      - The spike replacement method to use.  Options are 'avg'|'nan'\n";
	print "-S|--stddev      - The number of standard deviations +/- allowed\n";
	print "-P|--percent     - The sample to sample percentage variation allowed\n";
	print "-N|--number      - The maximum number of spikes to remove from the RRDfile\n";
	print "-D|--dryrun      - If specified, the RRDfile will not be changed.  Instead a summary of\n";
