...
if ($pid == 0) {
	...
	$info       = [];
	$rrdp_state = pcntl_sigwaitinfo([SIGUSR1, SIGTERM], $info);
	exit;
}
...
$rrdp_config['remote_clients'] = isset($rrdp_remote_clients) ? $rrdp_remote_clients : [];
$rrdp_config['remote_proxies'] = isset($rrdp_remote_proxies) ? $rrdp_remote_proxies : [];
...
while ($__server_listening) {
	...
	if ($ready) {
		foreach ($read as $read_socket_index => $read_socket) {
			...
			if ($read_socket_resource_id == $rrdp_client_resource_id) {
				...
				if ($rrdp_config['max_cnn'] > __sizeof($rrdp_clients)) {
					...
					if (array_key_exists($ip, $rrdp_config['remote_clients']) === true) {
						// setup IPC
						socket_create_pair(AF_UNIX, SOCK_STREAM, 0, $ipc_sockets);
						[$ipc_socket_parent, $ipc_socket_child] = $ipc_sockets;
				...
				if ($rrdp_config['max_admin_cnn'] > __sizeof($rrdp_admin_clients)) {
					...
					if (in_array($ip, ['127.0.0.1', 'localhost', '::1', '::ffff:127.0.0.1'], true)) {
						if (!in_array($socket_descriptor, $rrdp_admin_sockets, true)) {
							$socket_resource_id                      = rrdp_system__get_resource_id($socket_descriptor);
							$rrdp_admin_sockets[$socket_resource_id] = $socket_descriptor;
							$rrdp_admin_clients[$socket_resource_id] = ['socket' => $socket_descriptor, 'ip' => $ip, 'privileged' => false, 'logging_severity_console' => $rrdp_config['logging_severity_terminal'], 'logging_category_console' => $rrdp_config['logging_category_terminal'], 'debug' => false, 'type' => 'srv'];
						...
						$socket_resource_id                      = rrdp_system__get_resource_id($socket_descriptor);
						$rrdp_admin_sockets[$socket_resource_id] = $socket_descriptor;
						$rrdp_admin_clients[$socket_resource_id] = ['socket' => $socket_descriptor, 'ip' => $ip, 'privileged' => false, 'debug' => false, 'type' => 'msr'];
			...
			} elseif ($rrdp_replicator_pid !== 'undefined' && $read_socket_resource_id == $rrdp_clients[$rrdp_replicator_pid]['ipc']) {
				...
				if (isset($rrdp_admin_sockets[$read_socket_resource_id])) {
...
function rrdp_system__filter($output, $args) {
	...
	if (__count($args) >= 3 && $args[0] == '|' && in_array($args[1], ['i', 'e', 'b', 'I', 'E', 'B', 'exp'], true)) {
		$args[2]     = trim(implode(' ', array_slice($args, 2)), '"\'');
		$output_rows = explode(NL, $output);

		if (!is_array($output_rows)) {
			return $output;
		}
		$total_matches = 0;
		$output        = '';
...
function rrdp_system__global_console_logging_update() {
	...
	if (__count($selected_categories) > 0) {
		if (__count($selected_categories) > 1 && in_array('none', $selected_categories, true)) {
	...
	if ($rrdp_repl_master_pid) {
		...
		if (isset($rrdp_ipc_sockets[$key]) && rrdp_system__is_resource($rrdp_ipc_sockets[$key][1])) {
			@socket_write($rrdp_ipc_sockets[$key][1], serialize(['type' => 'reload_running_config', 'rrdp_config' => $rrdp_config]) . NL);
		}
	}
...
function rrdp_cmd__enable($socket, $args) {
	...
	if (!$invalid) {
		...
		if (in_array($rrdp_admin_clients[$socket_resource_id]['ip'], ['127.0.0.1', 'localhost', '::1', '::ffff:127.0.0.1'], true)) {
		...
		if ($socket === 'SIGTERM' || (isset($rrdp_admin_clients[$socket_resource_id]) && $rrdp_admin_clients[$socket_resource_id]['privileged'] === true)) {
			...
			foreach ($rrdp_clients as $child_pid => $client) {
				if ($socket === 'SIGTERM') {
					...
					if (isset($rrdp_ipc_sockets[$key]) && rrdp_system__is_resource($rrdp_ipc_sockets[$key][1])) {
						posix_kill($child_pid, SIGTERM);
						@socket_write($rrdp_ipc_sockets[$key][1], serialize(['type' => 'shutdown']) . NL);
					}
				}
		...
		switch($arg) {
			case 'add':
				if (__sizeof($args) == 3) {
					...
					if (filter_var($args[1], FILTER_VALIDATE_REGEXP, ['options' => ['regexp' => '/^$|^(102[4-9]|10[3-9]\d|1[1-9]\d{2}|[2-9]\d{3}|[1-5]\d{4}|6[0-4]\d{3}|65[0-4]\d{2}|655[0-2]\d|6553[0-5])$/']]) === false) {
						rrdp_system__socket_write($socket, '% Invalid PORT. Range: [1024-65535]' . NL);

						break;
					}

					if (filter_var($args[2], FILTER_VALIDATE_REGEXP, ['options' => ['regexp' => '/^([a-z0-9]{2}:){15}([a-z0-9]{2})$/']]) === false) {
					...
					$rrdp_config['remote_proxies'][$args[0]] = ['port' => $args[1], 'fingerprint' => $args[2]];
					...
					if (isset($rrdp_ipc_sockets[$key]) && rrdp_system__is_resource($rrdp_ipc_sockets[$key][1])) {
						@socket_write($rrdp_ipc_sockets[$key][1], serialize(['type' => 'reload_proxy_list']) . NL);
					}
					$key = $rrdp_clients[$rrdp_repl_slave_pid]['ipc'];
			...
			case 'remove':
				if (isset($args[0])) {
					if (isset($rrdp_config['remote_proxies'][$args[0]])) {
						...
						if (is_dir('./msr/' . $args[0])) {
							$files = array_diff(scandir('./msr/' . $args[0]), ['.', '..']);
	...
	if (!is_null($arg)) {
		switch($arg) {
			...
			case 'snmp':
				...
				if (is_numeric($level) && in_array($level, range(0,7), true)) {
			...
			case 'console':
				...
				if ($argument == 'level') {
					...
					if (is_numeric($level) && in_array($level, (($arg == 'terminal' | $arg == 'console') ? range(0,8) : range(0,7)), true)) {
						if ($arg == 'console') {
					...
					if (__count($categories) > 0) {
						...
						foreach ($categories as $category) {
							...
							if (array_key_exists($category, $logging_categories)) {
...
function handle_child_processes($ipc_sockets, $type, $ssock = false, $arg1 = false) {
	...
	if ($pid === 0) {
		...
		if (rrdp_system__is_resource($ssock)) {
			socket_close($ssock);
		}
		...
		$rrdp_status = ['type' => 'status', 'bytes_received' => 0, 'bytes_sent' => 0, 'queries_rrdtool_total' => 0, 'queries_rrdtool_valid' => 0, 'queries_rrdtool_invalid' => 0, 'rrd_pipe_broken' => 0, 'status' => 'RUNNING'];
